...
export async function activate(ctx) {
  ...
  const savedTimers = await ctx.storage.get('timers');
  ...
  ctx.commands.registerCommand('/timer', async (args) => {
    if (args.length === 0) {
      return {
        type: 'builtin',
        command: '/timer',
  ...
  ctx.commands.registerCommand('/timer:list', async () => {
  ...
  ctx.commands.registerCommand('/timer:clear', async () => {
...
function createTimer(ctx, seconds, label) {
...
function scheduleTimerNotification(ctx, timer) {
...
async function persistTimers(ctx) {
